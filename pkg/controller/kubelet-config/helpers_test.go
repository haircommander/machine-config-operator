...
func TestReserveSystemCPUs(t *testing.T) {
	testCases := []struct {
		...
		{
			name:                              "reservedSystemCPUs set with kube-reserved - should preserve kube-reserved",
			reservedSystemCPUs:                "0-1",
			initialSystemReservedCgroup:       "/system.slice",
			initialEnforceNodeAllocatable:     []string{kubeletypes.NodeAllocatableEnforcementKey, kubeletypes.KubeReservedEnforcementKey},
			expectedSystemReservedCgroup:      "",
			expectedEnforceNodeAllocatable:    []string{kubeletypes.NodeAllocatableEnforcementKey, kubeletypes.KubeReservedEnforcementKey},
			shouldDisableSystemReservedCgroup: true,
		},
	}
...
func TestSystemCgroupsMismatch(t *testing.T) {
	originalKubeConfig := &kubeletconfigv1beta1.KubeletConfiguration{
		MaxPods:                110,
		SystemReservedCgroup:   "/system.slice",
		EnforceNodeAllocatable: []string{kubeletypes.NodeAllocatableEnforcementKey, kubeletypes.SystemReservedCompressibleEnforcementKey},
	...
	kubeletConfig := &mcfgv1.KubeletConfig{
		ObjectMeta: metav1.ObjectMeta{
			Name: "test-systemcgroups-mismatch",
		},
		Spec: mcfgv1.KubeletConfigSpec{
			KubeletConfig: &runtime.RawExtension{
				Raw: userKubeletConfigRaw,
